...
class chara {
  ...
  calculateOrderValue(LoadFactor, LoadFactorReduce = 0) {
    ...
    let OrderValueRadix = Math.min(Math.max(124 - Math.abs(SPD / 2), 0), 100);
...
class chara {
    constructor(name, Element, StunCoff,
        HP, ATK, DEF, MAT, MDF, SPD, LUK,
        Skill,
        ATK_buff, DEF_buff, MAT_buff, MDF_buff, SPD_buff, LUK_buff,
        ATK_NextBuff, MAT_NextBuff,
        ...
        const x = [name, HP, ATK, DEF, MAT, MDF, SPD, Skill];
        ...
        this.DEF_buff = DEF_buff;
        this.MAT_buff = MAT_buff;
        this.MDF_buff = MDF_buff;
        this.SPD_buff = SPD_buff;
    ...
    //     let OrderValue = ((124 - (SPD / 2)) * (1 + (SPD_buff * 0.01) *  (1 + (SPD_buff * 0.01) * (1 + LoadFactorReduce ))));
    //     return OrderValue;
    // }

    turnStart() {
        this.changeStunGauge(-0.2)
    }
    ...
    calculateOrderValue(SPD = this.SPD, SPD_buff = [0],
        LoadFactor = this.LoadFactor, LoadFactorReduce = [0]) {
        let OrderValueRadix = Math.min(Math.max(Math.abs(124 - Math.abs(SPD / 2)), 0), 500);
        let OrderValue = Math.abs(OrderValueRadix * LoadFactor *
            (1 - LoadFactorReduce) * (1 - this.sum_BuffDebuff(SPD_buff)))
        return OrderValue;
    }
    ...
    InitOrderValue() {
        return this.calculateOrderValue(this.SPD, this.SPD_buff, 100, 0)
    }
...
timeline_test[0] = new timeline()

testChara[0] = new chara("test", "Element", "StunCoff",
    11, 22, 33, 44, 55, 66, 77,
    [
        [{ SkillType: "heal" }, { Skill: 1200 }, { LoadFactor: 30 }, { recast: 2 }, { ttk: 17 }],
        []
    ],
    "a", "", "", "", [0], "", ""
